...
async function loadPrivacyPage() {
  try {
    return await getPublicStaticPage("privacy");
  } catch {
    return null;
  }
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return buildLocalizedMetadata(locale, {
    title: (locale === "ar" ? page?.seo_title_ar || page?.title_ar : page?.seo_title_en || page?.title_en) || dict.public.legal.privacyTitle,
    description:
      (locale === "ar" ? page?.seo_description_ar : page?.seo_description_en) || dict.public.legal.privacyBody[0],
    path: `/${locale}/privacy`
  });
}
...
export default async function PrivacyPage({ params }: PageProps) {
  ...
  const title = (locale === "ar" ? page?.title_ar : page?.title_en) || dict.public.legal.privacyTitle;
  const paragraphs = paragraphsFrom(locale === "ar" ? page?.content_ar : page?.content_en, dict.public.legal.privacyBody);
